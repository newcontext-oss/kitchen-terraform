...
module Kitchen
  module Terraform
    module Transport
      # Doctor checks for transport configuration errors.
      class Doctor
        # #call executes the action.
        #
        # @param config [Hash] the configuration of the transport.
        ...
        def call(config:)
          ...
          if !::File.exist? client
            errors = true
            logger.error "#{instance_name}.transport.client '#{client}' does not exist"
          end
          if !::File.executable? client
            errors = true
            logger.error "#{instance_name}.transport.client '#{client}' is not executable"
          end
        ...
        # @return [Kitchen::Terraform::Transport::Doctor]
