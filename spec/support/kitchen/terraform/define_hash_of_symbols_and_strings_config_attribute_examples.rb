...
::RSpec
  .shared_examples "Kitchen::Terraform::DefineHashOfSymbolsAndStringsConfigAttribute" do |attribute:|
    include_context(
      ...
      context "when the config associates #{attribute.inspect} with a nonhash" do
        it_behaves_like(
          ...
          error_message: /#{attribute}.*must be a hash/,
          value: []
        )
      end
      ...
      context "when the config associates #{attribute.inspect} with a hash which has nonsymbol keys" do
        it_behaves_like(
          "the value is invalid",
          error_message: /#{attribute}.*keys must be symbols/,
          value: {"key" => "value"}
        )
      end
      ...
      context "when the config associates #{attribute.inspect} with a hash which has nonstring values" do
        it_behaves_like(
          "the value is invalid",
          error_message: /#{attribute}.*values must be strings/,
          value: {key: :value}
        )
      end
