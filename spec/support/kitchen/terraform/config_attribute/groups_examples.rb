...
::RSpec.shared_examples "Kitchen::Terraform::ConfigAttribute::Groups" do
  include_context "Kitchen::Terraform::ConfigAttribute", attribute: :groups do
    ...
    describe "the optional group attributes" do
      ...
      context "when the group associates :shell_command with a nonstring" do
        it_behaves_like "the value is invalid",
                        error_message: /groups.*0.*shell_command.*must be a string/, value: [{shell_command: 123}]
      end

      context "when the group associates :shell_command with a string which is empty" do
        it_behaves_like "the value is invalid",
