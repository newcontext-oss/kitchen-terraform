...
::RSpec.shared_examples "config attribute :lock_timeout" do
  include_context "Kitchen::Terraform::DefineConfigAttribute", attribute: :lock_timeout do
    context "when the config omits :lock_timeout" do
      it_behaves_like "a default value is used", default_value: "0s"
    end

    context "when the config associates :lock_timeout with a nonstring" do
      it_behaves_like "the value is invalid", error_message: /lock_timeout.*must be a string/,
                                              value: 123
    end
