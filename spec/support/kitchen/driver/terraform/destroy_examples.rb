...
::RSpec.shared_examples "::Kitchen::Driver::Terraform::Destroy" do
  context "when the action is a failure" do
    before do
      allow(described_instance.client).to receive(:apply_destructively).and_throw :failure, "failure"
    end
  ...
  context "when the action is a success" do
    before do
      allow(described_instance.client).to receive(:apply_destructively).and_throw :success, "success"
    end
