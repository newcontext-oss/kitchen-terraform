...
::RSpec.shared_examples "config attribute :state" do
  include_context "Kitchen::Terraform::DefineConfigAttribute", attribute: :state do
    ...
    context "when the config associates :state with a nonstring" do
      it_behaves_like "the value is invalid",
                      error_message: /state.*must be a string/,
                      value: 123
    end

    context "when the config associates :cli with an empty string" do
      it_behaves_like "the value is invalid",
                      error_message: /state.*must be filled/,
                      value: ""
    end
