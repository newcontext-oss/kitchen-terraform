...
::RSpec.describe ::Kitchen::Config::ApplyTimeout do
  describe ".call" do
    ...
    describe "value validation" do
      before do
        allow(plugin_class).to receive(:required_config).with(:apply_timeout).and_yield :apply_timeout, value, plugin
      end
      ...
      context "when the configuration associates :apply_timeout with a noninteger" do
      ...
      context "when the configuration associates :apply_timeout with an integer" do
    ...
    describe "defining the default value" do
      before do allow(plugin_class).to receive(:required_config).with :apply_timeout end

      after do described_class.call plugin_class: plugin_class end

      subject do plugin_class end

      it "defines the default value as 600" do is_expected.to receive(:default_config).with :apply_timeout, 600 end
    end
  end
end
