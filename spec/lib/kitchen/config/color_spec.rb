...
::RSpec.describe ::Kitchen::Config::Color do
  describe ".call" do
    ...
    describe "value validation" do
      before do
        allow(plugin_class).to receive(:required_config).with(:color).and_yield :color, value, plugin
      end
      ...
      context "when the configuration associates :color with a nonboolean" do
        ...
        it_behaves_like "the configuration is invalid" do let :error_message do /color.*must be boolean/ end end
      end
    ...
    describe "defining the default value" do
      before do allow(plugin_class).to receive(:required_config).with :color end

      after do described_class.call plugin_class: plugin_class end

      subject do plugin_class end

      it "defines the default value as true" do is_expected.to receive(:default_config).with :color, true end
    end
  end
end
