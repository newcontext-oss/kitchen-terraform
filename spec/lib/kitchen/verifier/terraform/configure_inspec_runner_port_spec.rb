...
::RSpec.describe ::Kitchen::Verifier::Terraform::ConfigureInspecRunnerPort do
  ...
  context "when the group associates :port with an object" do
    ...
    it "associates the options' 'port' with the group's :port" do
      is_expected.to be group_port
    end
  end
  ...
  context "when the group omits :port" do
    ...
    it "does not change the options' 'port'" do
      is_expected.to be options_port
    end
  end
end
