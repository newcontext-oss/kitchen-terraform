...
::RSpec.describe ::Terraform::PrepareOutputFile do
  let(:described_instance) { described_class.new file: '/output/file' }
  ...
  describe '#execute' do
    ...
    before do
      allow(::Pathname).to receive(:new).with('/output/file').and_return file

      allow(file).to receive(:parent).with(no_args).and_return parent_directory
